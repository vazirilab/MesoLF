...
% Comment by YZ: this is just a wrap function, which contains the update of the g, i.e.
% the AR coefficients
...
if ~optimize_b   %% don't optimize the baseline b
    ...
    %% initialization
    b = quantile(y, 0.15);
    [solution, spks, active_set] = oasisAR1(y-b, g, lam); % interesting, the solution is further updated by g?
...
for m=1:len_active_set
    ...
    tmp_hh = cumsum(tmp_h.*tmp_h);        % hh(k) = h(1:k)'*h(1:k)  % fixed by TN
    li = active_set(m, 4);
    ti = active_set(m, 3);
    idx = ti:(ti+li-1);
