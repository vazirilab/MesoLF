function [Mpr, shifts, bound, option_nr] = motion_correction(Yf, d1, d2, bin_width, outdir)
...
% metrics, on filtered and full movie
...
subplot(312); plot(1:T,cY,1:T,cM1);
    title('Correlation with mean frame, low-pass-filtered movie','fontsize',14,'fontweight','bold');
    legend('Raw data','Rigid motion correction');
subplot(313); plot(1:T,cYf,1:T,cM1f);
    title('Correlation with mean frame, unfiltered movie','fontsize',14,'fontweight','bold');
    legend('Raw data','Rigid motion correction');
...
option_nr.grid_size = [min(128, round(option_r.d1 / 4)), min(128, round(option_r.d2 / 4)), 1];
option_nr.mot_uf = [4, 4, 1];
option_nr.correct_bidir = false;
option_nr.overlap_pre = [20, 20, 1];
option_nr.overlap_post =[20, 20, 1];
option_nr.iter = 3;
...
legend('Raw data','Rigid motion correction','Non-rigid motion correction'); 
title('Correlation with mean frame, low-pass-filtered movie','fontsize',14,'fontweight','bold') 
set(gca,'Xtick',[],'XLim',[0,T-3])

ax2 = subplot(312); 
plot(shifts_x); 
hold on; plot(shifts_r(:,2)); 
title('Non-rigid displacements along x','fontsize',14,'fontweight','bold')
set(gca,'Xtick',[])

ax3 = subplot(313); 
plot(shifts_y); 
hold on; plot(shifts_r(:,1));
title('Non-rigid displacements along y','fontsize',14,'fontweight','bold')
