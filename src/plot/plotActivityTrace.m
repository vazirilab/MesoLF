...
% input parameters:
% inferred, filtered, raw (from the output of "signalExtraction.m"). One
% could choose df/f, f or df
...
K=size(inferred,1);
if isfield(plotControl,'maxNnum') maxNnum=min(plotControl.maxNnum, K);
else maxNnum=K; end
...
if normalization==1
    for idx=1:maxNnum
        if plotRaw  plot(t,raw(idx,:)/max(raw(idx,:))-idx*sep,'color',[0.3 0.3 0.3],'linewidth',2); end
    ...
    for idx=1:maxNnum
        if plotRaw  plot(t,raw(idx,:)-idx*sep,'color',[0.3 0.3 0.3],'linewidth',2); end
...
h=gca;
if ~rollingView 
    axis(h,[0 max(t) -sep*(maxNnum+1) sep]);
else
    idx=1;
    displayNum=20;
    while idx+displayNum<maxNnum
