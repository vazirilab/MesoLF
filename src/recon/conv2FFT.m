function y = conv2FFT(x, m, zeroImageEx, exsize)
% original version
xsize = size(x);
msize = size(m);
fsize = xsize;
...
if isa(m, 'gpuArray')
    mc = gpuArray(mc);
    me = gpuArray(me);
    m1 = gpuArray(exindex(m, mc(1):me(1), mc(2):me(2), 'circular'));
else
    m1 = exindex(m, mc(1):me(1), mc(2):me(2), 'circular');
end

y = real(ifft2(fft2(x) .* fft2(m1)));   
...
xout = zeros(size(zeroImage), 'like', zeroImage);  % trying to get rid of occasional CUDA error 'named symbol not found'
i_rng = 1:size(xin, 1);  % trying to get rid of occasional CUDA error 'named symbol not found'
j_rng = 1:size(xin, 2);  % trying to get rid of occasional CUDA error 'named symbol not found'
xout(i_rng, j_rng) = xin;
end
