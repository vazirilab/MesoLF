function [valid_seg, discard_seg] = hierarchical_clustering(segments, ...
    ...
    output_dir, discard_component_image_save, valid_component_image_save)
...
% size_w = 450;'
if nargin < 8
    discard_component_image_save = true;
end
if nargin < 9
    valid_component_image_save = true;
end
...
%% do clustering
fprintf('Compute linkage tree and cut\n')
...
textprogressbar('Merge valid segments ')
for i = 1 : N_seg
...
save(fullfile(output_dir, [datestr(now, 'YYmmddTHHMM') '_valid_segments.mat']), 'valid_seg', '-v7.3');
save(fullfile(output_dir, [datestr(now, 'YYmmddTHHMM') '_discard_segments.mat']), 'discard_seg', '-v7.3');

%% do fuse, use label function, XYCZT order
if valid_component_image_save
    ...
    % TODO: Creating these diagnostic plots is quite slow...
    textprogressbar('Create RGB labels for valid segments ');
    for i = 1 : valid_num
    ...
    textprogressbar(' done')
    hyperstack_write(fullfile(output_dir, [datestr(now, 'YYmmddTHHMM') '_img_valid_segment.tif']), img_valid);
    hyperstack_write(fullfile(output_dir, [datestr(now, 'YYmmddTHHMM') '_valid_in_std_recon_img.tif']), valid_in_std_recon_img);
end

%% Plot discarded components: do fusion, use label function, XYCZT order
if discard_component_image_save
