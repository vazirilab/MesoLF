function AY = mm_fun_lomem(A,Y,chunk_size,run_paralel,FOV)
% Strongly based on https://github.com/flatironinstitute/CaImAn-MATLAB/blob/master/utilities/mm_fun.m
% Slightly modified to avoid duplication/data type conversion of variable Y if it is single
...
elseif isobject(Y);
    filetype = 'mem';  % it is a memory-mapped array
    sizY = size(Y,'Y');
    T = sizY(end);
else % regular array loaded in memory
    filetype = 'mat';
    %Y = double(Y);  % this can blow up memory by factor 3 if Y is single
    %(original var remains, outside of this function)
    if ~ismatrix(Y)
